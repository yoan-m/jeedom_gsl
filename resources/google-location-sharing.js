...
if (isUndefined(arg1_user)) { emitVerbose('error',"Error, missing user argument (Google e-mail account).\n"+"Usage: "+node_binary+" "+this_script+" user pass"); process.exit(5); }
if (isUndefined(arg2_pass)) { emitVerbose('error',"Error, missing pass argument.\n"+                        "Usage: "+node_binary+" "+this_script+" user pass"); process.exit(5); }
...
function main() {
  emitVerbose('debug','Starting google shared locations adapter');
...
function connectToGoogle(callback) {
  connectFirstStage(function(err) {
    if (err) {
      emitVerbose('error','First stage auth error');
      if (callback) callback(err);
    } else {
      connectSecondStage(function(err) {
        if (err) {
          emitVerbose('error','Second stage (auth user) error');
          if (callback) callback(err);
        } else {
          connectThirdStage(function(err) {
            if (err) {
              emitVerbose('error','Third stage (auth password) error');
              if (callback) callback(err);
            } else {
              connectFourthStage(function(err) {
                if (err) {
                  emitVerbose('error','Fourth stage (locator) error');
                  if (callback) callback(err);
                } else {
...
function connectFirstStage(callback) {
  emitVerbose('debug',"First stage, connecting to Google ...");
  ...
  request(options_connect1, function(err, response, body){
    if (err || !response) {
      emitVerbose('error',err);
      emitVerbose('error','Connection failure.');
     
      if (callback) callback(true);
    } else {
      emitVerbose('debug','Connection successful. Response: ' + response.statusMessage);      
      ...
      if (response.statusCode !== 200) {
        ...
        if (response.hasOwnProperty('headers') && response.headers.hasOwnProperty('set-cookie')) {
          ...
          emitVerbose('debug','Connection successful. Saved connection cookies.');          
          if (callback) callback(false);
        } else {
          emitVerbose('error','No cookie found.'); 
...
function connectSecondStage(callback) {
  emitVerbose('debug',"Second stage, sending E-Mail address ..."); 
...
function connectThirdStage(callback) {
  emitVerbose('debug',"Third stage, sending password ...");
    ...
    if (err || !response) {
      ...
      if (response.statusCode !== 302) {
        ...
        if (response.hasOwnProperty('headers') && response.headers.hasOwnProperty('set-cookie')) {
          ...
          emitVerbose('debug','Authentication successful, received new location URL: ' + google_fourth_location_url); 
...
function getSharedLocations(callback) {
  getCookieHeader('google.com', function(err, cookieheader) {
    ...
    request(options_map, function(err, response, body) {
      if (err || !response) {
        emitVerbose('error','Connection to google maps failure.'); 
        if (callback) callback(true);
      } else {
        emitVerbose('debug','Connection successful. Response: ' + response.statusMessage); 
        if (response.statusCode !== 200) {
          emitVerbose('error','Connection successful, but authorization failure (wrong password?)!'); 
          if (callback) callback(true);
        } else {
          emitVerbose('debug','Connection successful, and authorization OK.'); 
